-- Copied from https://github.com/billperegoy/elm-form-validations
-- and modified to basically work better and not cause build errors.
-- Licensed under BSD-3-Clause license by Bill Peregoy.

module Forms exposing (..)

{-| # Forms

This library performs form validation for HTML forms. A user can define a list
of form elements and define validations for each element. Each time the form
is updated, validations are automatically run and validation results are
updated for each form element as well as the overall form.
...
{-| Initializes a form.
    ...
    Accepts a list of (name, validator) tuples and returns a Form.
-}
...
{-| Given a Form, a field name and a field value and returns
    a new form containing the update. This is generally called
...
{-| Returns the current value of any form field.
    Returns the empty string if an invalid form name is given.

-}
formValue : Form -> String -> String
formValue form name =
    formValueWithDefault "" form name


{-| Returns the current value of any form field.
    If that field does not yet have a value, returns the given default.
    Returns the empty string if an invalid form name is given.

-}
formValueWithDefault : String -> Form -> String -> String
formValueWithDefault default form name =
    let
        lookupValue =
            Dict.get name form.elements
    in
        case lookupValue of
            Just value ->
                Maybe.withDefault default value.input
...
validateSingle formElement =
    validateField (Maybe.withDefault "" formElement.input) formElement.validator
...
{-| Validates existence of a form vale.
...
{-| Validates that an integer field is within a specified numeric range.
-}
