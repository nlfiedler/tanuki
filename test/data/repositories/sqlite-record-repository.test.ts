...
describe('SqliteRecordRepository', function () {
  const settingsRepository = new EnvSettingsRepository();
  const sut = new SqliteRecordRepository({ settingsRepository });
  ...
  test('should retrieve document by checksum', async function () {
    ...
    doc.importDate = new Date(2003, 7, 30, 12, 0);
    // act
  ...
  test('should fetch assets in batches', async function () {
    ...
    expect(batch1[9]?.key).toEqual('harry');

    const [batch2, cursor2] = await sut.fetchAssets(cursor1, 10);
    expect(batch2).toHaveLength(8);
    expect(batch2[0]?.key).toEqual('janet');
    expect(batch2[7]?.key).toEqual('wingtim');
