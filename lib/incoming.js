...
function updateAssetFields (asset, incoming) {
  if (_.isString(incoming['tags'])) {
    // TODO: this condition can be removed once REST API is retired
    // Tags may come in as a comma-separated string, so split, sort, unique.
    let list = incoming.tags.split(',').map(t => t.trim())
    asset.tags = _.sortedUniq(list.sort())
  } else if (_.isArray(incoming['tags'])) {
    asset.tags = _.sortedUniq(incoming.tags.sort())
  }
  if (_.isString(incoming['location'])) {
    // If location is given, overwrite document field.
    asset.location = incoming.location
  }
  if (_.isString(incoming['caption'])) {
    asset.caption = incoming.caption
  ...
  // TODO: can be removed once REST API is retired
  if (_.isString(incoming['user_date'])) {
    incoming.date = incoming.user_date
  }
  if (_.isString(incoming['date'])) {
    if (incoming.date.length > 0) {
      // pass the original asset for getting the best date, otherwise
      // you get the 'user_date', which we are trying to set right now
      asset.user_date = mergeUserDateWithBest(incoming.date, asset)
    } else {
      // wipe out the user date field if an empty string is given
