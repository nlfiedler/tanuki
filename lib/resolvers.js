...
const BigInt = new GraphQLScalarType({
  name: 'BigInt',
  description: 'The `BigInt` scalar type represents signed 53-bit integers.',
...
function coerceBigInt (value) {
  if (value === '') {
    throw new TypeError('BigInt cannot represent non 53-bit signed integer value: ""')
  }
  const num = Number(value)
  if (Number.isNaN(num) || num > Number.MAX_SAFE_INTEGER || num < Number.MIN_SAFE_INTEGER) {
    throw new TypeError(`BigInt cannot represent non 53-bit signed integer value: ${value}`)
  }
