...
async function byYear (year, summarize = true) {
  const startKey = new Date(year, 0, 0, 0, 0).getTime()
  const endKey = new Date(year + 1, 0, 0, 0, 0).getTime()
...
function filterByYear (years, rows) {
  return rows.filter((row) => {
    const rowYear = new Date(row['value'][0]).getFullYear()
    return years.findIndex((y) => y === rowYear) !== -1
  })
}
