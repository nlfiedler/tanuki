...
/**
 * Generate a resized image to fit the given height.
 *
 * @param {string} mimetype - the mimetype of the file.
 * @param {string} filepath - path of the file.
 * @param {number} height - height in pixels of the desired image.
 ...
 *          resolves to null if no thumbnail available.
 */
async function generateWideThumb (mimetype, filepath, height) {
  if (mimetype.startsWith('video/')) {
    return generateVideoThumbnail(filepath, null, height)
  } else if (mimetype.startsWith('image/')) {
    // resize the image to the given height, convert to jpeg
    let bits = await sharp(filepath)
      .resize(null, height)
      .withoutEnlargement()
      .toFormat('jpeg')
