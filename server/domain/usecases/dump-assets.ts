...
import { metadataToDocument } from 'tanuki/server/data/repositories/asset-metadata-codec.ts';
...
export default ({
  ...
  return async function* (batchSize: number) {
    let cursor = null;
    while (true) {
      const [assets, next] = await recordRepository.fetchAssets(
        cursor,
        batchSize
      );
      for (const entry of assets) {
        ...
        if (entry.location !== null) {
          if (
            ...
            location = {
              l: entry.location.label,
              c: entry.location.city,
              r: entry.location.region
            };
          }
        }
        yield {
          ...
          dimensions: null,
          metadata: metadataToDocument(entry.metadata)
        };
      }
      if (assets.length === 0) {
        break;
