...
**** TODO consider how to show bloc errors using the snackbar
***** that is, do not disturb whatever is already on the screen
***** zorigami does this I think
***** use a bloc listener to render the error message
***** normal widget still needs something to render in case of error
...
***** add event to =AssetBrowserBloc= that sets before date filter (clears selected year)
***** add event to =AssetBrowserBloc= that sets after date filter (clears selected year)
