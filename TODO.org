...
**** TODO enhance browse page
***** DONE add pagination controls
***** TODO display thumbnails with captions in a grid instead of list view
****** try using =Image.network= to load the thumbnails via URL
****** try using =Wrap= to do what =flex-flow: row wrap;= does in CSS
****** or perhaps use =GridView.count= with a fixed count of 3 for now
******* eventually can compute a better number based on viewport size
******* see https://flutter.dev/docs/development/ui/layout for various layouts
******* also ^^^ gives a way to deal with "portrait" and "landscape" modes
***** TODO maybe offer a control to change page size
***** TODO try to get the Enter key in page input field to submit
***** TODO try to remove the =SizedBox= in the pagination control
...
**** [promote this group up one level and prioritize tasks]
**** browse and search
...
***** try using the date range picker function in material
...
****** one row for tags, another for locations, years/date range, etc
