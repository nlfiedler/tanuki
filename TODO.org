...
**** DONE stop current docker container
**** DONE use export script to get dump of current database
**** DONE use import binary to create new database
**** DONE deploy new docker container with new backend
**** TODO find and remove the video thumbnails (there will be a =.mov= and =.jpg= with the same id)
***** just use shell commands and find them, there aren't that many
*** TODO remove mujina from mac mini
*** TODO seems like the default sort order is backward
**** add support for sorting by identifier or checksum in search usecase
**** need to add sort params to =SearchParams= in graphql module
**** default the sort order to descending date
**** graphql tests need to sort by checksum or identifier (dates are non-deterministic)
