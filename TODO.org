* Action Items
** Pending
*** TODO selecting seems to cause everything to re-render (because of videos?)
*** display options: have an option that makes the images really big (single column, huge images)
...
** Browse
*** TODO retain search parameters and page number when navigating back and forth
**** could use a signal above the routes, use =provide_context()= to send down the tree
**** could use local storage for each signal value and hydrate on page load
*** TODO make the selectors row sticky when scrolling
...
** Dismissing dropdowns (pagination, color scheme)
*** use =on_click_outside= in =leptos-use= crate
** Details
*** center smaller images
*** input validation of some sort, probably
*** completion for tags (like home screen search)
*** somehow offer completion of location even though its in 3 separate fields
...
*** details formatting and presentation
...
*** Show addition file metadata (TIFF, EXIF, JFIF, IPTC)
** Uploads
...
*** provide (un)select all buttons to toggle selections
*** access the operation popup menu (FAB) to edit selected items
...
*** when an asset cannot produce a thumbnail, server a placeholder
...
**** add audio playback support using the =audio= tag
