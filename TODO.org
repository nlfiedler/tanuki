...
*** TODO bulk edit support [3/7]
- [X] add a query that returns all unique location label, city, region values as separate lists
- [X] add use case that performs modifications to assets (add tags, remove tags, etc)
- [X] add resolver mutation to invoke the use case
- [ ] add edit page with form for making changes
- [ ] add query text field for finding assets
- [ ] allow selection of multiple assets
- [ ] enable ~Commit~ button when selections and form inputs are valid
*** TODO search cache for =scan-assets= usecase
**** clear the cache in update, edit use cases
*** TODO store various selections in local storage [15/19]
- [X] (recent) page size
...
*** TODO asset edit form needs to allow easier editing of tags
**** currently the signal accessor/setter interfere with editing
*** TODO ~Replace~ button on details page
...
*** TODO drop zone on upload page needs to use appropriate color depending on color mode
**** in light mode, the dotted rectangle is invisible
...
**** could be the database view takes a second to update
**** change CouchDB repo to wait a second on ~pending~ call if changes were made, reset flag once pending finishes
...
*** TODO bun docker 1.3.4 ~module not found~ error
