...
** TODO change dump/load to operate in batches
*** TODO change =RecordRepositoryImpl::dump()= in repo to call =fetch_assets()= to get assets in batches
*** TODO change =RecordRepositoryImpl::load()= to read batches of lines at a time, convert to =Vec<Asset>=
*** TODO add =store_assets()= to data sources that receives a =Vec<Asset>= argument
...
**** load timing for 21,502 records on M1 w/USB-HDD (Zbackup)
| progress   | time   |
|------------+--------|
| loc dedupe | 1m 10s |
|            |        |
**** load timing for 21,502 records on M1 w/SSD
| progress   | time   |
|------------+--------|
| initial    | 3.235s |
| loc dedupe | 2.020s |
