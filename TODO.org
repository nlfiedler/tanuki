...
*** add a =has:= predicate to the query support
*** use =has:original_date= and =has:user_date= to find assets that have both (and =is:video=)
*** write a usecase and GraphQL mutation to fix existing records
**** find any videos with null =originalDate= and try to determine the actual date-time
*** what about the user date field? should it be cleared if original date is corrected?
