...
**** TODO consider managing everything server-side
- search: client starts via GraphQL ~searchBrowse~ with current search parameters
- scan: client starts via GraphQL ~scanBrowse~ with current scan query
- server caches a page of results along with all parameters for search/scan
- server returns a unique browsing ~key~ for subsequent ~browsePrev~ and ~browseNext~ calls
- client invokes ~browsePrev~ and ~browseNext~ with ~key~ (stored in local storage)
- server manages position with the search results page
- server fetches prev/next page of search results as needed
- server returns values indicating if there is a previous and next asset to be viewed or not
**** TODO browse page
**** TODO search page
...
** Pending
*** bug: saving changes often fails to fetch updated search results
**** reloading the page a second later works
**** updates are successful, the page is not caching anything
...
** Load is not scalable
*** load operation reads entire file into memory, converts all lines to JSON, converts JSON to Asset
*** load will soak up memory before dumping all assets into the database at one time
*** consider how to process in chunks to avoid loading everything into memory
** Hosting externally
