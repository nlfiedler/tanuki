* Tanuki
** TODO Try making the selector dropdown toggle the item instead of just adding
** [#A] Enable basic bulk update of user date
...
*** shrink the dropdown area, it expands to the size of the value
**** makes it impossible to remove locations from the input chips
