...
*** Phase 2 goals [0/5]
- [ ] details/edit page
- [ ] ~import~ usecase and button on uploads page
- [ ] search query language
- [ ] search page
- [ ] search caching?
*** Phase 3 goals [0/3]
- [ ] read creation time from mpeg files
- [ ] read date from RIFF/AVI files
- [ ] look for HEIC support
*** Overall action Items
**** TODO fix ~Error: You are not authorized to access this db.~ when invoking =/records/dump=
***** happens after leaving it alone for 3 hours or less
***** stop/start the container to restore the connection
***** default session timeout is 600 seconds
***** CouchDB repository needs to reconnect if too much time has elapsed
***** wip code
#+begin_src javascript
  async ensureConnected() {
    // TODO: add a new property 'lastRequestTime' that records the last time this function was called
    // TODO: check if new Date().getTime() - lastRequestTime > COUCHDB_SESSION_TIMEOUT * 1000
    // TODO: if the elapsed time exceeds the configured timeout, reconnect
    // TODO: update lastRequestTime to new Date().getTime()
  }
#+end_src
**** TODO put the usecase jsdoc comments on the actual function
...
***** updates are successful, page quietly fails to refresh
**** TODO catch and log GraphQL errors (at least for ~count~)
**** TODO linting [0/5]
...
**** TODO read original date-time from videos
...
*** GraphQL resolver would inject the thumbnail/preview/raw URLs into search results, assets
...
**** asset ID is still useful (includes time of import, file extension; guaranteed unique)
...
**** server could redirect the client to the blob store
...
*** offering an alternative like SQLite would guarantee app design stays generic
*** CouchDB is neat, but growing increasingly dependent on that may eventually prove too limiting
