...
*** DONE serve the existing ReasonML frontend
**** DONE add handler for static files
**** DONE add default handler for all non-matching routes
**** DONE add endpoints for graphql and graphiql
**** DONE add CORS middleware
**** DONE ensure logging middleware is set up
**** DONE generate the =graphql_schema.json= using the new API
**** DONE fix the ReasonML code related to GraphQL
**** DONE convert file upload to use HTTP endpoint
***** see =Upload.re= in commit =5974201= for how it used to be
**** DONE add endpoint to get the thumbnail and previews
...
**** DONE add endpoint to get the raw asset and its media type
***** DONE read the media type from the database
***** DONE get the asset path and return the raw file in the response
***** DONE should return 404 for missing asset, not 500
***** DONE =/asset/= needs to respond to =HEAD= as well as =GET=
**** DONE ensure web endpoints use =web::block()= appropriately
**** DONE fix date handling in thumbnails
**** DONE fix date handling on show asset page
**** DONE fix file size handling on show asset page
**** DONE year selector is not working
**** DONE search by date is not working
**** DONE fix the user date handling on edit page
*** TODO repository =blob_path()= should take only an asset id, not an asset
*** TODO write tests for the http endpoints
**** c.f. https://actix.rs/docs/testing/
*** TODO try to clean up the =main.rs= TODO items
*** TODO probably do not need =get_media_type()=
*** TODO index mapper should not emit empty values
**** asset with no location emitted a blank and it shows up in the list of locations
