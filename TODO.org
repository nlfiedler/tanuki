...
*** TODO Pinterest style ~masonry grid~ (columns) layout; preserves aspect ratio
*** TODO search page should have some gallery features as home page
*** DONE add ~loading="lazy"~ to =img= tags
*** DONE captions should be displayed using an overlay on hover
*** TODO GraphQL to serve image ~width/height~ and/or ~aspectRatio~
...
**** would need to join search result with asset table
*** some of these will need a join on another database with extra information for each asset
**** query can pass =include_docs=, but then cached results will be much larger
**** maybe GraphQL can do a ~batch~ fetch of many records at once to avoid 1+N query
**** =POST /{db}/_all_docs?include_docs=true= and provide multiple document keys
*** https://piccalil.li/blog/a-simple-masonry-like-composable-layout/
*** https://blog.logrocket.com/responsive-image-gallery-css-flexbox/ (bottom of page)
*** https://nazanin-ashrafi.hashnode.dev/create-an-image-gallery-using-flexbox (bottom of page)
*** Masonry Photo Gallery Grid (actually justified rows) -- https://codepen.io/michydev/pen/ZEYdOEZ
*** Responsive Photo Gallery with CSS Grid -- https://codepen.io/optimisticweb/pen/pvzoELv
*** Responsive Photo Gallery Grid with Lightbox -- https://codepen.io/alchatti/pen/NOdzLO
*** grid of 640x640 images with caption info (date, location/filename) (a la PhotoPrism)
