* Action Items
** Quick import directly from the phone, tablet
*** Ensure web interface works well for small screens
**** Flutter Folio example app that works well for all screen sizes
***** c.f. https://flutter.gskinner.com
*** Support easy import of photos from phone collection via web interface
...
#+end_src
*** unable to clear the before/after dates; form doesn't have a reset button?
**** probably need to overlay our own ~close~ button to reset the field
**** the date/time picker field already has has ~reset~ icon but does not appear
...
**** TODO see the new =ChipsInput= which has a focus node property
**** TODO c.f. https://github.com/danvick/flutter_form_builder/issues/313#issuecomment-861586184
...
**** see work-around in issue #313 of https://github.com/danvick/flutter_form_builder
#+BEGIN_SRC
FocusNode focusNode;
...
#+END_SRC
**** see another comment and work-around to the same issue
: Ran into the same problem. After testing different combinations of
: interactions, I discovered that if I restart the application, and interact
: with the FormBuilderDateTimePicker control first, it works. However, if I
: interact with a FormBuilderSegmentedControl, and then try to click on
: FormBuilderDateTimePicker control, the date time picker throws the focus
: exception.
:
: The solution that worked for me, without making any modifications to the
: original flutter_form_builder code was to update my calls to
: FormBuilderSegmentedControl, and set the focusNode property. See below.
#+begin_src dart
FormBuilderSegmentedControl(
...
focusNode: FocusNode(), // added attribute
...
)
#+end_src
: There is an issue w/ how FormBuilderField is disposing the focusNode. It
: becomes an issue if application is hot reloaded. You may have to do a hot
: restart instead.
