...
***** c.f. https://medium.com/flutter/learning-flutters-new-navigation-and-routing-system-7c9068155ade
***** see the =onPopPage()= of the =Navigator= for a way to update state
***** use =Navigator= in =MyApp= to set up the set of pages
***** combine the various blocs so the =onPopPage()= can fire update events
****** either combine the blocs themselves into one, or use =MultiBlocProvider=
***** alternative: can use =NavigatorObserver= with =MaterialApp= to watch for changes
...
**** browse and search
***** consider copying =FormBuilderChipsInput= and adding a drop-down selector
***** user can type or use selector to add attributes to the field
...
*** TODO Try building for Windows (see if video player works)
