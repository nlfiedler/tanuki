...
***** blob repo
***** data source
**** should use =juniper::ID= where =ID= is expected
**** add a =relative_path()= to =BlobRepository= to get =filepath= for asset
**** queries
...
****** should enforce a maximum value for =count=
****** =SearchResult= is supposed to have =mimetype= but it was missing before
