...
*** location data is stored in images using Exif
**** Exif tags with =GPS= prefix
- GPSLatitudeRef (N or S)
- GPSLatitude (degrees, minutes, seconds)
- GPSLongitudeRef (E or W)
- GPSLongitude (degrees, minutes, seconds)
*** TODO change ~location~ property to a struct
**** new structure of the ~location~ property
| Name   | Description              |
|--------+--------------------------|
| label  | current string value     |
| city   | name of city             |
| region | state, province, country |
**** region should be two-letter state code, if available, to save space
**** region may be the country if it has no states or provinces (e.g. Israel, Norway, many others)
**** no need for country, state/province is already clear, saves space, usually same value for many assets
**** deserialize: convert plain ~location~ string into the new structure
**** serialize: if structure only has ~label~ then only write a string for ~location~
**** c.f. serde string/struct example https://serde.rs/string-or-struct.html
*** TODO GraphQL: extract GPS location from Exif of images
**** add a new ~gps~ field to ~Asset~ that is populated only on demand (for efficiency)
**** use the =kamadak-exif= crate to read the =GPS= fields
*** TODO utilize a service or database to convert GPS to ~city~ and ~region~
*** TODO add GraphQL mutation to scan all assets and update city/region if possible
*** TODO update =all_locations= index to emit keys for ~city~ and ~region~
*** TODO show tiny map of where photo was taken on details page
*** DONE add GraphQL query to scan all assets to report available information
*** TODO find out if videos have location data (=mp4= crate might help)
**** Preview.app will display location information for videos
*** existing asset analysis results
| property            | count |
|---------------------+-------|
| totalAssets         | 11132 |
| missingFiles        |     5 |
...
| hasExifData         | 10451 |
| hasGpsCoords        |  2204 |
| hasOriginalDatetime | 10127 |
| hasOriginalTimezone |   876 |
...
** [#C] Read timezone info from Exif tags when displaying asset details
