...
** TODO ML labels
*** DONE model licenses
*** DONE publish models and mapping to tanuki releases
*** TODO compare =/synthetic= output of tanuki to the 3 fixtures from namazu
*** TODO make sure there is a ~namazu~ detector on the tanuki side that calls =/synthetic= route
*** DONE document =label:<class>= query predicate
*** TODO document =person:id= query predicate (with face recognition)
*** TODO add advice about deployment (CouchDB + namazu)
*** TODO add =FACE_STORE_PATH= to settings and docker deployment
** TODO PDF support
...
** TODO edit page: clear field, tags and Confirm button not updated
** Albums
*** add to navbar
...
| label    | label:beach           | match on an ML-derived label   |
