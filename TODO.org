* Action Items
** TODO switch from =rusty_ulid= to =ulid=
** TODO upgrade =juniper= dependency, involves significant API changes
** TODO make graphql and usecases =async= throughout, remove the =block_on()= function
*** seemingly juniper graphql operations can be either async or sync
** production instance tasks
*** process all of the incoming
*** process all of wifey photos in Photos.app
*** do something with the assets that have ~fix_the_date~ tag
*** fix orientation on many images
**** lot of the images from ~jarabacoa~, wedding, hong kong, need rotating
**** anything on or before 2016 probably
...
#+end_src
** open source
*** TODO delete =CHANGELOG.md=
*** TODO delete =COPYRIGHT=
*** TODO copy =LICENSE= from zorigami
*** TODO delete ~Product~ section from =TODO.org=
*** TODO move ~Deployments~ section in =TODO.org= to wiki
*** TODO change =license= to =MIT= in =Cargo.toml=
** idea for rotating images
*** test using =magick_rust= to see if it preserves metadata
*** create a simple EXIF parser that scan for =Orientation= and changes the value if it exists
*** if not found, insert the =Orientation= at the end of the EXIF header
...
**** =image= crate currently discards metadata (https://github.com/image-rs/image/issues/1901)
...
* Product
** Project Management
*** consider a tool like https://github.com/plankanban/planka
** Productization
*** Windows support
