* Action Items
** BUG: home page fails to load anything (GraphQL and images)
*** cause? seems to happen after the docker engine is updated and containers restart
*** browser console shows error about cross-origin-opener-policy, but that always happens
*** fix: redeploying everything seems to fix it
*** maybe try simply restarting the app container alone
*** no indication of problems in the log files
*** could be that CouchDB started later than app and hence the app gets stuck
*** TODO look for ~database initialization error~ in log output
*** TODO maybe =stayAlive()= needs to catch any errors and invoke =initialize()= if error
**** maybe extract some of =initialize()= into a new =connect()= to just do the connect part, stayAlive() calls that
** Help
...
** TODO photo gallery modes
*** DONE Flickr style ~justified rows~ layout; clean lines; portraits get truncated
*** DONE Pinterest style ~masonry grid~ (columns) layout; preserves aspect ratio
*** DONE dropdown selector options: justified rows, masonry, original card grid
*** DONE add ~loading="lazy"~ to =img= tags
*** DONE captions should be displayed using an overlay on hover
*** DONE design solution for retaining more asset details
*** DONE run the following locally to test the operations (nothing changed)
*** DONE dump/load use cases should round-trip ~metadata~
*** DONE import new assets locally to test the behavior via GraphQL
*** DONE use search result =metadata.displayHeight= and =.displayWidth= to help column layout
*** TODO grid of 640x640 images with caption info (date, location/filename) (a la PhotoPrism)
**** details below the thumbnail with tiny icons (see screenshot on desktop)
- label / city / year
- user friendly date-time + TZ
- camera make and model
- camera settings (aperature, shutter speed, etc)
- format (JPEG), dimensions, file size
- original file name
- location place, city, region
*** DONE can videos show a thumbnail instead of placeholder?
*** TODO search page should have same gallery features as home page
*** TODO deploy and run the backfill operations on chihiro
*** TODO run back-fill: fix original dates
*** TODO run back-fill: backfill image metadata
*** TODO run back-fill: backfill video metadata
*** TODO use https://blurha.sh to have quick placeholders while images load
**** clear the cache and evaluate whether image loading is a problem
**** would need to join search result with asset table
...
*** zoom level slider to change size of thumbnails
** TODO PDF viewer support
...
*** extract metadata, populate database
** PhotoPrism style one-word ML tag
*** note that the data generated by AI/ML is referred to as ~synthetic data~
*** in the PhotoPrism grid layout, the first word in the multiline caption is an ML label
*** examples: "Greenhouse", "Ring", "Building", "Garden", "Bush", "Flower"
*** this label word is followed by ~/ <city> / <year>~
*** Machine learning
