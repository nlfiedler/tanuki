...
** Optimizations
*** use =include_bytes!= to pull placeholder image into code from a file
*** find and remove =allow(dead_code)=
*** find and remove =allow(unused)=
*** consider if the search filtering should be pushed further down for performance
**** database layer could employ a minheap to retain the N elements that match the filter and sort order
*** consider how to cache query results to avoid scanning index again for pagination
*** apparently RocksDB provides =set_prefix_same_as_start()= option for iterator
*** why are the queries returning results in which =asset_id= has the =asset/= prefix?
** Pending
