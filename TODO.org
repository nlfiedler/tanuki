...
***** query tags
***** search for assets
***** lookup asset via checksum
***** fetch asset details
**** repositories
***** record database
***** asset store
**** action plan
***** define a =UseCase= trait with a =call()= function
****** generic for =Type= that is returned from =call()=
****** generic for =Params= that defines parameters for operation
***** each use case has a constructor that takes dependencies
****** e.g. repository implementation
***** write unit tests (using mocks for data layer) for the domain layer
***** implement the data model
****** models wrap the domain entities to allow for use of =#[derive()]=
****** implement serialization for entities via =serde=
****** implement the =From= and =Into= for the entities
***** implement the data repository
***** implement the data source
***** write unit tests (using mocks for the database) for data layer
