* Action Items
** Bugs
*** some png files fail to produce thumbnails (most work okay)
** Help
...
** Details
*** use local storage to define asset id, set from home page, read/write from asset page
...
**** create new ~browse~ parameters for ~asset~ page
***** encapsulate everything in one struct
- query string
- selected tags
- selected locations
- selected year
...
- sort order
- current asset index
***** use JSON serde to store this as local variable, CBOR for server function
***** results display invokes =onclick= fn attribute, implemented in ~home~ and ~search~ pages
***** ~home~ and ~search~ pages construct the appropriate ~browser~ params
***** ~browse~ params are stored in local storage, then ~asset~ page is opened
***** ~asset~ page uses ~browse~ params from local storage to invoke server fn
***** add server function that returns the asset details given the ~browse~ params
***** add server functions for moving ~forward~ and ~backward~ within the overall results
***** server functions will manage paging the results
***** ~scan~ and ~search~ use cases will continue caching as usual
**** test the ~save~ and ~replace~ functions
