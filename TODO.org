...
***** record database
***** blob store
*** TODO decide how to deal with image orientation
**** image orientation was previously being corrected during import
**** makes more sense to only correct when serving the thumbnails/previews
***** already processing the image at that point anyway
***** and we do not care about preserving the original metadata for thumbnails
***** and how the client renders the original asset is entirely application dependent
**** c.f. https://magnushoff.com/articles/jpeg-orientation/
**** https://crates.io/crates/image (MIT) strips all metadata
**** https://crates.io/crates/libvips (MIT) introduces many dependencies
**** https://github.com/vadixidav/exifsd (MIT) can copy data, but may not be working
...
**** detect orientation in original image, resize, then auto-orient
**** https://crates.io/crates/image (MIT)
***** =image= is easier to use than =libvips= which requires more management
***** c.f. =image::DynamicImage::thumbnail()= which preserves aspect ratio
**** https://crates.io/crates/libvips (MIT)
***** c.f. =libvips::ops::thumbnail= and friends
