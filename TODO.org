...
*** remote blob store would serve assets directly to client (e.g. S3)
...
*** GraphQL resolver would inject the thumbnail/asset URLs into results
...
*** avoid SeaweedFS, it stores files in 32gb volumes
*** TODO build something new in Rust (namazu)
**** build using =actix-web= and =image= crates
**** assets are immutable, can only post new assets and delete old ones
**** asset identifier is effectively the tag for that unique asset forever
**** use base64 encoded paths for assets to serve existing content w/o migration
**** start with Rust tanuki =main.rs=, add in thumbnail code
**** support PUT, GET, DELETE for assets on =/assets= endpoint
**** =PUT= to existing path results in a 409, success returns 201
**** write integration tests
**** support thumbnails via query parameters (like SeaweedFS)
#+begin_src
http://localhost:8080/3/01637037d6.jpg?height=200&width=200
http://localhost:8080/3/01637037d6.jpg?height=200&width=200&mode=fit
http://localhost:8080/3/01637037d6.jpg?height=200&width=200&mode=fill
#+end_src
**** support ETag and Cache-Control headers
***** ETag should be a unique value for the asset and the query parameters
***** should be safe to use just the ULID filename instead of the entire asset id
***** =Cache-Control: public, max-age=31536000=
**** use actix =NamedFile= to handle =Range= requests
