...
*** TODO Consider splitting the web interface into its own repo
Need to consider how to have the ReasonML backend code separate from the
webui code. Right now they would both live in the =src= directory? Seems
confusing, would need some other sensible code layout.
