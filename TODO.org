...
**** change CouchDB repo to query ~pending~ twice if =putAsset= was called, reset flag once pending finishes
...
*** TODO rename =createIfMissing()= in record repository to =initialize()=
