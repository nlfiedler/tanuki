...
|------------+------------------------+---------------------+--------------|
| path       | file path/identifier   | varchar primary key |           64 |
...
| city   | text                |
| region | text                |
*** DONE derive SQL queries for each of the repository functions
...
#+end_src
**** DONE query_by_tags() -> Vec<SearchResult>
***** for now, do a full table scan and match tags in Rust
#+begin_src sql
SELECT path, filename, mimetype, tags, label, city, region,
       first_some(user_date,orig_date,imported) AS datetime
FROM assets
LEFT JOIN locations ON assets.location = locations.id;
#+end_src
**** DONE query_by_locations() -> Vec<SearchResult>
#+begin_src sql
SELECT assets.*, locations.* FROM assets
LEFT JOIN locations ON assets.location = locations.id
WHERE locations.label = ? OR locations.city = ? OR locations.region = ?;
#+end_src
...
#+end_src
**** DONE query_before_date() -> Vec<SearchResult>
***** write app-defined fn named =first_some()= that selects first Some argument
...
***** define index on ~assets~ for the =first_some()= of date fields in preferred order
***** query on ~assets~ as usual, should use index (use =EXPLAIN= to validate index is used)
#+begin_src sql
SELECT path, filename, mimetype, label, city, region,
       first_some(user_date, orig_date, imported) AS datetime
FROM assets
LEFT JOIN locations ON assets.location = locations.id
WHERE datetime < ?;
#+end_src
**** DONE query_after_date() -> Vec<SearchResult>
...
#+end_src
**** DONE query_date_range() -> Vec<SearchResult>
***** do whatever was done for query_before_date()
#+begin_src sql
SELECT path,filename,mimetype,location,imported,user_date,orig_date from assets WHERE path = ?;
#+end_src
**** DONE query_newborn() -> Vec<SearchResult>
...
FROM assets
LEFT JOIN locations ON assets.location = locations.id
WHERE tags = NULL, caption = NULL, label = NULL;
#+end_src
**** DONE all_locations() -> Vec<LabeledCount>
***** query and convert results to =Vec<LabeledCount>= somehow
***** can use logic of =count_all_keys()= in mokuroku
#+begin_src sql
SELECT * from locations;
#+end_src
**** DONE raw_locations() -> Vec<Location>
***** query and convert results to =Vec<Location>=
#+begin_src sql
SELECT * from locations;
#+end_src
**** DONE all_years() -> Vec<LabeledCount>
***** use best date, convert to =Vec<LabeledCount>=
***** can use logic of =count_all_keys()= in mokuroku
#+begin_src sql
SELECT user_date, orig_date, imported FROM assets;
#+end_src
**** DONE all_tags() -> Vec<LabeledCount>
***** query, split value on commas, convert to =Vec<LabeledCount>=
***** can use logic of =count_all_keys()= in mokuroku
#+begin_src sql
SELECT tags from assets;
#+end_src
**** DONE all_media_types() -> Vec<LabeledCount>
***** query and collect into =Vec<LabeledCount>=
***** can use logic of =count_all_keys()= in mokuroku
#+begin_src sql
SELECT mimetype from assets;
#+end_src
**** DONE fetch_assets() -> Vec<Asset>
...
#+end_src
*** TODO action plan
**** DONE consider how unreferenced rows in =locations= table could eventually be removed
...
**** TODO consider how to build indexes for tags, locations, dates
***** otherwise many queries will do full table scans
**** TODO use =explain= to ensure indexes are used
**** TODO consider when to clean up the database (e.g. find orphaned locations)
