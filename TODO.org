...
*** DONE search query language
*** TODO bulk edit support [0/3]
- [ ] add a query that returns all unique location label, city, region values as separate lists
- [ ] add use case that performs modifications to assets (add tags, remove tags, etc)
- [ ] add resolver mutation to invoke the use case
*** TODO search cache for =scan-assets= usecase
*** TODO rename proper-cased file names in git (~/rename-files.sh)
