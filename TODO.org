...
** Browse
*** retain search parameters and page number when navigating back and forth
...
*** dark mode is enabled in Bulma using the =attr:data-theme= on the =Html= tag in =mod.rs=
*** c.f. https://github.com/leptos-rs/example-darkmode (very elaborate example)
*** use =bulma-switch= from https://bulma.io/extensions/ for the HTML element
*** use =gloo_storage= crate to store theme selection in =LocalStorage=
