...
*** TODO Utilize the Clean Architecture
...
4) export
5) edit
**** define the models and use cases
***** asset record
***** location record (label, gps, etc)
***** secondary index record (location, media type, filename, date)
**** the graphql schema and resolvers are the presentation layer
**** the database and models are the data layer
**** the glue that makes it all work is the domain layer
***** this includes the logic for processing imports, massaging data, etc
...
*** TODO Use dart =coverage= to measure test effectiveness
**** c.f. https://github.com/dart-lang/coverage
**** need a good way to summarize the output (look for lcov tools)
...
*** TODO Clean Architecture: do it
**** business rules in redux-saga code become the use cases
