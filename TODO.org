...
** [#B] why do the search results include a ~location~ value?
*** filename and date/time are displayed in the ui
*** media type might be used for something?
*** but what is location used for?
** [#C] Quick rotation fix for certain images
...
** [#A] Dump and load
*** Add GraphQL mutations for dumping and loading the database
**** provides another mechanism for backup, resetting the database disk usage
**** use JSON format, one asset per line, makes it easy to sed/awk the result
