...
** [#A] remove the ~location~ property from =SearchResult=, it is unused
** [#B] change the ~newborn~ index to have ~city~ and ~region~ instead of ~mimetype~ and ~location~
*** need a different struct for the index value of ~newborn~ (not =SearchResult=)
** [#C] Migrate the production database
1) dump to a file
2) stop the container
3) rename the database directory
4) create an empty database directory
5) start the container
6) confirm it is empty
7) load the dump file
8) confirm search is working
9) confirm recents are still recent
