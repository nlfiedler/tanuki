...
**** TODO fix broken tests after running =flutter pub upgrade=
***** look for any known issues that match the error message
...
***** be sure to sort the incoming files by filename for consistency
...
**** TODO eventually try to get drag and drop to work for uploading files
