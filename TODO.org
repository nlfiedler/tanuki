...
*** TODO add mutation to scan all assets and update city/region if possible
*** TODO add mutation that sets =city= and =region= based on matching location =label=
**** allow clearing the ~label~ in favor of just ~city~ and ~region~
*** TODO doc: setting up mapping services
**** Google Maps
- create a google cloud account and enable the ~Geocoding API~
- create a new API key that is restricted to the ~Geocoding API~
- not any API key will do, it must be enabled for the specific API
...
*** frontend will send either a ~rotate CW~ or ~rotate CCW~ request to backend
...
*** TODO GraphQL: change =Asset= definition to have =Location= struct for =location= property
