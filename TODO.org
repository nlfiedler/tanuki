...
*** set the caption
*** advanced search: search for assets with location ~label~ but not other fields
**** e.g. has ~home~ but not ~castro valley~ and not ~san lorenzo~
**** query: ~loc:home not loc:'castro valley' not loc:'san lorenzo'~
**** the edit use case previously had this type of feature
*** replace form elements with a single text field and ~Search~ button
**** show a help text below the input field with basic syntax
** Navbar
...
** Advanced Search
*** efficiency: use a ~cursor~ of sorts to paginate results
**** since the advanced search bascially scans all records, use a cursor to return pages of results
**** return a tokenized version of the cursor as an opaque ~continuation~ token
**** usecase will take an =Option<Token>= that it uses to continue the database iteration
*** Advanced queries textual operators and operands
**** copy the perkeep style query support from mujina (=src/query.js=)
**** operands can be enclosed in single or double quotes
**** operations can be grouped with parentheses
**** operations; no ~and~ operator because that is the default
...
| not    | not format:jpeg    | inverse of next operation    |
