...
|------------+------------------------+----------------------------------------+--------------|
| key        | file path/identifier   | varchar not null primary key           |           64 |
| hash       | sha256 digest          | varchar not null unique collate nocase |           64 |
| filename   | original name          | varchar not null                       |           12 |
| filesize   | byte length            | ubigint not null                       |            8 |
| mimetype   | media type             | varchar not null collate nocase        |           10 |
| caption    |                        | varchar                                |            0 |
| tags       | list of tags           | varchar[]                              |           20 |
| location   | FK on locations.id     |                                        |              |
| imported   | datetime of import     | timestamp_s not null                   |           64 |
...
***** can only be done for new assets, conflicts will fail
***** DuckDB can easily handle 10,000 rows at a time
***** c.f. https://duckdb.org/docs/stable/data/appender
**** TODO add tests for DuckDB in =data_sources_test.rs=
