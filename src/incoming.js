...
/**
 * Save the readable stream data to a temporary file.
 *
 * @param {string} filename name of original file.
 * @param {stream.Readable} stream incoming stream.
 * @returns {Promise<string>} Promise resolving to the path of the saved file.
 */
async function receiveAsset (filename, stream) {
  ...
  return new Promise((resolve, reject) => {
    ...
    writable.on('error', (err) => cleanup(err))
