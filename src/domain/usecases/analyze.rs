...
impl Analyze {
    ...
    fn examine_image(&self, blob_path: &Path, counts: &mut Counts) -> Result<(), Error> {
        ...
        if let Ok(exif) = reader.read_from_container(&mut buffer) {
            ...
            if let Some(_) = exif.get_field(exif::Tag::DateTimeOriginal, exif::In::PRIMARY) {
                counts.has_original_datetime += 1;
            }
            if let Some(_) = exif.get_field(exif::Tag::OffsetTimeOriginal, exif::In::PRIMARY) {
            ...
            if let Some(_) = exif.get_field(exif::Tag::GPSLatitude, exif::In::PRIMARY) {
